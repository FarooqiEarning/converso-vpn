...
export const authApi = {
  ...
  googleLogin: (accessToken: string) => api.post('/auth/google', { accessToken }),
};

export const usersApi = {
  getProfile: () => api.get('/users/me'),
  updateProfile: (data: { fullName?: string; avatarUrl?: string }) => api.patch('/users/me', data),
...
export const nodesApi = {
  ...
  getRecommended: (latency?: number) => api.get('/nodes/recommend', { params: { latency } }),
...
export const devicesApi = {
  ...
  update: (id: string, data: { deviceName: string }) => api.patch(`/devices/${id}`, data),
  switchNode: (id: string, nodeId: string) => api.patch(`/devices/${id}/node`, { nodeId }),
